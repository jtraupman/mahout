...
public class DistributedConjugateGradientSolver extends ConjugateGradientSolver implements Tool
{
   ...
   * @param b              Vector b
   * @param lambda         Value of the lambda parameter which scales an identity matrix added to A 
   *                       to prevent non-singularity (cf. ridge regression)
   * @param isSymmetric    If true, then the system (A + lambda*I)x = b is solved.
   *                       If false, then the system (A'A + lambda*I)x = b is solved 
  ...
  public Vector runJob(Path inputPath, 
    ...
    LinearOperator linop = matrix;
    if (!isSymmetric) {
      linop = new SquaredLinearOperator(linop);
    }
    if (lambda != 0.0) {
      linop = new DiagonalOffsetLinearOperator(linop, lambda);
    }
    
    return solve(linop, b, preconditioner, maxIterations, maxError);
  }
  ...
  public int run(String[] strings) throws Exception
  {
    ...
    Vector x = runJob(inputPath, tempPath, numRows, numCols, b, lambda, isSymmetric, null, maxIterations, maxError);
