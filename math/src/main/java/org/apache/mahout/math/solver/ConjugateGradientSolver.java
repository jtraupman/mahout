...
public class ConjugateGradientSolver
{
  ...
  private static final TimesFunction times = new TimesFunction();
   ...
   * 
   */
  public Vector solve(LinearOperator a, Vector b) {
   ...
   * 
   */
  public Vector solve(LinearOperator a, Vector b, Preconditioner precond) {
  ...
  public Vector solve(LinearOperator a, 
                      Vector b, 
    ...
    while (Math.sqrt(residualNormSquared) > maxError && iterations < maxIterations) {
      ...
      if (iterations == 1) {
        ...
        updateDirection.assign(times, beta);
        conditionedResidual.addTo(updateDirection);
      }
